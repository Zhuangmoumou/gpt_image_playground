...
function upsertTask(userId: string, task: Record<string, unknown>) {
  ...
  const existing = db.prepare('SELECT revision FROM tasks WHERE user_id = ? AND id = ?').get(userId, id) as { revision: number } | undefined
  db.prepare(`
    ...
    ON CONFLICT(user_id, id) DO UPDATE SET
      ...
      deleted_at = NULL
  `).run(id, userId, JSON.stringify(task), status, (existing?.revision ?? 0) + 1, createdAt, now)
}
...
function upsertImage(userId: string, image: z.infer<typeof storedImageSchema>) {
  ...
  db.prepare(`
    ...
    ON CONFLICT(user_id, id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at,
      deleted_at = NULL
  `).run(image.id, userId, saved.sha256, saved.storagePath, saved.mimeType, image.width ?? null, image.height ?? null, image.source ?? 'upload', createdAt, now)
}
...
function upsertThumbnail(userId: string, thumbnail: z.infer<typeof thumbnailSchema>) {
  ...
  db.prepare(`
    ...
    ON CONFLICT(user_id, image_id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at
  `).run(thumbnail.id, userId, saved.storagePath, thumbnail.width ?? null, thumbnail.height ?? null, thumbnail.thumbnailVersion ?? 1, Date.now())
}
...
function upsertConversation(userId: string, conversation: Record<string, unknown> & { id: string }) {
  ...
  const existing = db.prepare('SELECT revision FROM agent_conversations WHERE user_id = ? AND id = ?').get(userId, conversation.id) as { revision: number } | undefined
  db.prepare(`
...
function applySyncPayload(userId: string, payload: SyncPayload) {
  db.transaction(() => {
    if (payload.settings !== undefined) upsertUserSettings(userId, payload.settings)
...
function getSnapshot(userId: string) {
  ...
  const tasks = (db.prepare('SELECT task_json FROM tasks WHERE user_id = ? AND deleted_at IS NULL ORDER BY created_at DESC').all(userId) as TaskRow[])
    .map((row) => jsonColumn<Record<string, unknown>>(row.task_json, {}))

  const images = (db.prepare('SELECT id, storage_path, mime_type, width, height, source, created_at FROM images WHERE user_id = ? AND deleted_at IS NULL ORDER BY created_at DESC').all(userId) as ImageRow[])
    .map((row) => ({
      id: row.id,
  ...
  const thumbnails = (db.prepare('SELECT image_id, storage_path, width, height, thumbnail_version FROM thumbnails WHERE user_id = ?').all(userId) as ThumbnailRow[])
    .map((row) => ({
      id: row.image_id,
  ...
  const agentConversations = (db.prepare('SELECT conversation_json FROM agent_conversations WHERE user_id = ? AND deleted_at IS NULL ORDER BY updated_at DESC').all(userId) as ConversationRow[])
