...
export default function HelpModal({ appMode, onClose }: HelpModalProps) {
  ...
  return createPortal(
    <div
      ...
      onClick={onClose}
    >
      <div className="absolute inset-0 bg-black/30 backdrop-blur-sm glass-overlay animate-overlay-in" />
      <div
        ref={modalRef}
        className="relative z-10 w-full max-w-md rounded-3xl border border-white/50 bg-white/95 glass-panel p-5 shadow-2xl ring-1 ring-black/5 animate-modal-in dark:border-white/[0.08] dark:bg-gray-900/95 dark:ring-white/10 flex flex-col max-h-[85vh] custom-scrollbar"
