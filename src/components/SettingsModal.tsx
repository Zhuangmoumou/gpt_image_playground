...
const ZIP_DOWNLOAD_ROUTE_OPTIONS: Array<{ route: ZipDownloadRoute; label: string; description: string }> = [
  { route: 'task-selection', label: '任务列表 > 多选', description: '主页中框选、Ctrl/⌘ 点选或移动端滑动选中任务后的“下载选中”。' },
...
所有配置、任务记录、图片和 Agent 对话均以当前账号的服务端数据为准。导入和导出会直接读写服务端数据；浏览器本地缓存只用于显示和兼容旧数据。
        ...
        {showZipDownloadRouteManager && createPortal(
          ...
          >
            ...
            <div
              className="relative z-10 flex max-h-[85vh] w-full max-w-md flex-col rounded-3xl border border-white/50 bg-white/90 shadow-[0_8px_40px_rgb(0,0,0,0.12)] ring-1 ring-black/5 backdrop-blur-xl animate-confirm-in dark:border-white/[0.08] dark:bg-gray-900/90 dark:shadow-[0_8px_40px_rgb(0,0,0,0.4)] dark:ring-white/10 sm:max-h-[90vh]"
            ...
            >
              ...
              <div ref={zipDownloadRouteScrollBoundaryRef} className="custom-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto px-6 py-2">
