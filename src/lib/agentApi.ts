...
async function postAgentResponses(
  ...
  if (profile.useServerSideRequests !== false) {
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    const response = await postAgentResponses(settings, profile, body, controller.signal)
...
export async function callAgentConversationTitleApi(opts: {
  ...
  try {
    ...
    const response = await postAgentResponses(settings, profile, {
      model: profile.model || settings.model,
