...
export function detectKeyColorFromPixels(data: Uint8ClampedArray, width: number, height: number): string {
  ...
  for (let x = 0; x < width; x += 1) {
    ...
    borderIndices.push((height - 1) * width + x)
  }
  for (let y = 1; y < height - 1; y += 1) {
    borderIndices.push(y * width)
    borderIndices.push(y * width + width - 1)
  }
  ...
  for (const index of borderIndices) {
    ...
    const greenDist = Math.sqrt((red - greenRgb.r) ** 2 + (green - greenRgb.g) ** 2 + (blue - greenRgb.b) ** 2)
    const magentaDist = Math.sqrt((red - magentaRgb.r) ** 2 + (green - magentaRgb.g) ** 2 + (blue - magentaRgb.b) ** 2)
