...
describe('interrupted OpenAI running tasks', () => {
  it('marks legacy and OpenAI running tasks as interrupted', () => {
    ...
    const serverBackgroundRunning = task({ id: 'server-running', apiProvider: 'openai', serverJobId: 'job-1', status: 'running', createdAt: 5_000, finishedAt: null, elapsed: null })
    const doneTask = task({ id: 'done-task', apiProvider: 'openai', status: 'done' })

    const result = markInterruptedOpenAIRunningTasks([legacyRunning, openAIRunning, falRunning, customAsyncRunning, serverBackgroundRunning, doneTask], now)
    ...
    expect(result.tasks.find((item) => item.id === 'server-running')).toEqual(serverBackgroundRunning)
