...
import {
  ...
  getServerTask,
  getUserSettings,
  listServerAgentConversations,
  listServerTasks,
  patchServerTask,
  replaceServerAgentConversations,
  saveServerTask,
  saveUserSettings,
  uploadServerImage,
} from './lib/serverApi'
...
async function cacheServerImageReference(id: string, source: 'upload' | 'generated' | 'mask' = 'generated') {
  const url = getImageUrl(id)
  const stored = await getImage(id)
  if (!stored || stored.dataUrl !== url) {
    await putImage({
      id,
      dataUrl: url,
...
async function persistImageDataUrlToServer(
  dataUrl: string,
  source: 'upload' | 'generated' | 'mask',
  options: { storeOriginalDataUrl?: boolean } = {},
) {
  const uploaded = await uploadServerImage(dataUrl, source)
  await putImage({
    id: uploaded.image.id,
    dataUrl: options.storeOriginalDataUrl ? dataUrl : uploaded.image.url,
    createdAt: uploaded.image.createdAt,
    source,
  })
  cacheImage(uploaded.image.id, dataUrl)
...
export async function ensureImageCached(id: string): Promise<string | undefined> {
  ...
  return cacheServerImageReference(id)
}
...
async function replaceStoredAgentConversations(conversations: AgentConversation[]) {
  const persistable = conversations.map(getPersistableAgentConversation)
  if (useStore.getState().authUser) {
    await replaceServerAgentConversations(persistable)
    return
  }
  await replaceAgentConversations(persistable)
}
...
interface AppState {
  ...
  resetLocalAccountState: () => Promise<void>
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      resetLocalAccountState: async () => {
        imageCache.clear()
        thumbnailCache.clear()
        dismissAllTooltips()
        await Promise.all([clearImages(), dbClearTasks(), dbClearAgentConversations()])
...
function putTask(task: TaskRecord): Promise<unknown> {
  const persistable = getPersistableTask(task)
  if (useStore.getState().authUser) {
    return saveServerTask(persistable)
  }
  return dbPutTask(persistable)
}
...
async function completeRecoveredFalTask(task: TaskRecord, result: Awaited<ReturnType<typeof getFalQueuedImageResult>>) {
  ...
  for (const dataUrl of result.images) {
    const imgId = await persistImageDataUrlToServer(dataUrl, 'generated')
...
export async function initStore() {
  ...
  const [{ settings, params }, { tasks: storedTasks }, storedAgentConversationsRaw] = await Promise.all([
    getUserSettings(),
    listServerTasks(),
    authUser ? listServerAgentConversations().then((result) => result.conversations) : getAllAgentConversations(),
  ])
  ...
  const storedAgentConversations = normalizeAgentConversations(storedAgentConversationsRaw)
  ...
  const referencedImageIds = Array.from(referencedIds)
  await Promise.all(referencedImageIds.map((id) => cacheServerImageReference(id)))
  ...
  for (const img of persistedInputImages) {
    ...
    if (!dataUrl) continue
    restoredInputImages.push({ ...img, dataUrl })
  }
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  if (maskDraft) {
    try {
      ...
      const uploadedMask = await uploadServerImage(maskDraft.maskDataUrl, 'mask')
      maskImageId = uploadedMask.image.id
      await putImage({
        id: maskImageId,
        dataUrl: uploadedMask.image.url,
        createdAt: uploadedMask.image.createdAt,
        source: 'mask',
      })
      cacheImage(maskImageId, uploadedMask.image.url)
  ...
  const uploadedInputImages: InputImage[] = []
  for (const img of orderedInputImages) {
    if (img.dataUrl.startsWith('data:')) {
      const uploaded = await uploadServerImage(img.dataUrl, 'upload')
      inputIdMap.set(img.id, uploaded.image.id)
      await putImage({
        id: uploaded.image.id,
      ...
      uploadedInputImages.push({ id: uploaded.image.id, dataUrl: uploaded.image.url })
    } else {
      inputIdMap.set(img.id, img.id)
      await cacheServerImageReference(img.id, 'upload')
      uploadedInputImages.push(img)
    }
  }
  if (maskTargetImageId) maskTargetImageId = inputIdMap.get(maskTargetImageId) ?? maskTargetImageId
  orderedInputImages = uploadedInputImages
  useStore.getState().setInputImages(uploadedInputImages)
...
export async function submitAgentMessage() {
  ...
  if (maskDraft) {
    try {
      ...
      maskImageId = await persistImageDataUrlToServer(maskDraft.maskDataUrl, 'mask', { storeOriginalDataUrl: true })
    } catch (err) {
      if (!inputImages.some((img) => img.id === maskDraft.targetImageId)) {
  ...
  const serverBackedInputImages: InputImage[] = []
  for (const image of orderedInputImages) {
    const imageId = await persistImageDataUrlToServer(image.dataUrl, 'upload', { storeOriginalDataUrl: true })
    serverBackedInputImages.push({ id: imageId, dataUrl: image.dataUrl })
    if (image.id === maskDraft?.targetImageId) {
      maskTargetImageId = imageId
    }
  }

  orderedInputImages = serverBackedInputImages
...
async function executeAgentRound(
  ...
  try {
    ...
    const completeAgentImageTask = async (image: AgentApiResultImage, rawResponsePayload?: string) => {
      ...
      const imgId = await persistImageDataUrlToServer(image.dataUrl, 'generated')
    ...
    const usesApiProxy = activeProfile.useServerSideRequests === false && (activeProfile.apiProxy ?? requestSettings.apiProxy)
...
async function executeTask(taskId: string) {
  ...
  if (!useServerSideRequests && taskProvider !== 'fal' && !isAsyncCustomProviderTask(requestSettings, taskProvider, task.inputImageIds.length > 0)) {
  ...
  try {
    ...
    if (!useServerSideRequests && latestTask.apiProvider === 'fal' && latestFalRequestInfo && isFalConnectionRecoverableError(err)) {
      updateTaskInStore(taskId, {
        status: 'error',
      ...
      scheduleFalRecovery(taskId)
    } else if (!useServerSideRequests && latestCustomTaskInfo && isFalConnectionRecoverableError(err)) {
      updateTaskInStore(taskId, {
        status: 'error',
...
export function updateTaskInStore(taskId: string, patch: Partial<TaskRecord>, options: { persist?: boolean } = {}) {
  ...
  const changed = Object.keys(patch).some((key) => current[key as keyof TaskRecord] !== next[key as keyof TaskRecord])
  if (!changed) return
  ...
  if (options.persist !== false) {
    void patchServerTask(taskId, next)
  }
}
...
export async function removeMultipleTasks(taskIds: string[]) {
  ...
  setTasks(remaining)
  useStore.getState().setSelectedTaskIds(selectedTaskIds.filter((id) => !toDelete.has(id)))

  try {
    await deleteServerTasks(taskIds)
    const stillUsed = new Set<string>()
    for (const task of remaining) {
      addTaskReferencedImageIds(stillUsed, task)
    }
    addAgentReferencedImageIds(stillUsed)
    addInputDraftReferencedImageIds(stillUsed, galleryInputDraft)
    for (const img of inputImages) stillUsed.add(img.id)
    ...
    setTasks(tasks)
    useStore.getState().setSelectedTaskIds(selectedTaskIds)
...
export async function removeTask(task: TaskRecord) {
  ...
  try {
    ...
    for (const item of remaining) {
      addTaskReferencedImageIds(stillUsed, item)
    }
    addAgentReferencedImageIds(stillUsed)
    addInputDraftReferencedImageIds(stillUsed, galleryInputDraft)
    for (const img of inputImages) stillUsed.add(img.id)
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  ...
  if (options.clearTasks) {
    await clearServerTasks()
    if (useStore.getState().authUser) {
      await replaceServerAgentConversations([])
    }
    await dbClearTasks()
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    const tasks = options.exportTasks ? (await listServerTasks()).tasks : []
    ...
    if (options.exportTasks) {
      ...
      for (const id of allImageIds) {
        const response = await fetch(getImageUrl(id), { credentials: 'same-origin' })
        ...
        const thumbnailPath = `thumbnails/${id}.${thumbnailExt}`
        thumbnailFiles[id] = {
          path: thumbnailPath,
          width: thumbnail.width,
          height: thumbnail.height,
  ...
  try {
    ...
    if (options.importTasks && data.tasks && data.imageFiles) {
      ...
      for (const [id, info] of Object.entries(data.imageFiles)) {
        ...
        const uploaded = await uploadServerImage(dataUrl, info.source)
        imageIdMap.set(id, uploaded.image.id)
        await putImage({
          id: uploaded.image.id,
          dataUrl: uploaded.image.url,
          createdAt: uploaded.image.createdAt,
        ...
        cacheImage(uploaded.image.id, uploaded.image.url)
        importedImageIds.push(uploaded.image.id)
      }

      for (const [oldId, info] of Object.entries(data.thumbnailFiles ?? {})) {
        const mappedId = imageIdMap.get(oldId)
      ...
      for (const task of data.tasks) {
        await saveServerTask({
          ...task,
          inputImageIds: task.inputImageIds.map((id) => imageIdMap.get(id) ?? id),
          maskTargetImageId: task.maskTargetImageId ? imageIdMap.get(task.maskTargetImageId) ?? task.maskTargetImageId : task.maskTargetImageId,
          maskImageId: task.maskImageId ? imageIdMap.get(task.maskImageId) ?? task.maskImageId : task.maskImageId,
          outputImages: task.outputImages.map((id) => imageIdMap.get(id) ?? id),
        })
      }
...
export async function createInputImageFromFile(file: File): Promise<InputImage | null> {
  ...
  const uploaded = await uploadServerImage(dataUrl, 'upload')
  await putImage({
    id: uploaded.image.id,
    dataUrl,
    createdAt: uploaded.image.createdAt,
  ...
  cacheImage(uploaded.image.id, dataUrl)
  return { id: uploaded.image.id, dataUrl }
}
...
export async function addImageFromUrl(src: string): Promise<void> {
  ...
  cacheImage(uploaded.image.id, dataUrl)
  useStore.getState().addInputImage({ id: uploaded.image.id, dataUrl })
}
